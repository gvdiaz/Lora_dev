  ...
  while(1)
  {
    if(digitalRead(USERKEY)==0)
    {
      ...
      else
      {
        ...
        for(int i=0;i<50;i++)
        {
          adcvalue += analogReadMilliVolts(adcin);
          delay(1);
        }
        digitalWrite(37, 0);
        volt=adcvalue*4.9/50*1.05;
        packet ="battery:"+String(volt);
        factory_display.clear();
        if(volt<BATTER_MIN|volt>BATTER_MAX)
        {
          factory_display.drawString(0, 18, packet);
          factory_display.drawString(0, 30, "ADC test XX");
          factory_display.display();
        }
        else 
        {
          factory_display.drawString(0, 18, packet);
          factory_display.drawString(0, 30, "ADC test OK");
          factory_display.display();
          delay(500);
